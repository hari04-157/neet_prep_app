...
MY_GOOGLE_KEY = "AIzaSyAfGw2eXQzIvShLCdXDIhpPDtBq1GDRhxk" 
    ...
    if os.path.exists(csv_path):
        try:
            ...
            for index, row in df.iterrows():
                ...
                if explanation.lower() == "nan" or explanation == "": explanation = "Explanation not available."
...
def submit():
    ...
    feedback = "Keep pushing! You're getting there."
    badge = "Student"
    if prec > 90 and attempted > 0: feedback = "🎯 You are a Sniper! Incredible precision."; badge = "Sniper"
    elif speed < 15 and prec < 60: feedback = "⚠️ Too fast! Accuracy > Speed."; badge = "Speedster"
    elif acc > 80: feedback = "🏆 Top Rank Material! So proud of you."; badge = "Topper"
...
def chat_ai():
    ...
    prompt = f"""
    You are Hari, a loving boyfriend helping Harini study for NEET.
    Question: {context}
    Doubt: {user_query}
    ...
    try:
        ...
        return jsonify({"reply": "My AI brain is offline temporarily."})

@app.route('/api/speak_clone', methods=['POST'])
def speak_clone():
    data = request.json
    text = data.get('text', '')
    reference_audio = "hari.wav"
    output_filename = "static/ai_reply.wav"
    if os.path.exists(output_filename): os.remove(output_filename)
    
    voice_generated = False
    if os.path.exists(reference_audio):
        try:
            client = Client("fffiloni/xtts-v2-streaming") 
            result = client.predict(text, "en", reference_audio, "", False, api_name="/predict")
            shutil.copy(result[1], output_filename)
    ...
    if not voice_generated:
        ...
        except: return jsonify({"error": "Audio failed."})
